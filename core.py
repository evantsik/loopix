...
class SphinxPacker(object):
    ...
    def take_nodes_routing(self, nodes, drop_flag, type_flag, video_flag):
        nodes_routing = []
        for i, node in enumerate(nodes):
            delay = self.generate_random_delay(self.config.EXP_PARAMS_DELAY,video_flag)
    ...
    def decrypt_sphinx_packet(self, packet, key):
        header, body = packet
        ret = sphinx_process(self.sec_params, key, header, body)
        (tag, info, (new_header, new_body)) = ret
