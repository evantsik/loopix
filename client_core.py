...
class ClientCore(object):
    ...
    def pack_video_message(self, message, receiver, path):
        header, body = self.packer.make_sphinx_packet(receiver, path, message,video_flag=True)
        log.msg("[%s] > Packed video message." % self.name)
        return header, body
